...
pub fn runInterruptTests() !void {
    ...
    irq_vblank, irq_stat = PPU.cycle(&ppu, &memory.memory);
    ...
    memory.memory.memory[mem_map.interrupt_flag] = 0x00;
    ppu.lcd_stat = .{ .mode_0_select = true, .mode_2_select = true }; // HBlank, OAMScan
    // ppu.lyCounter = 0;
    ppu.lcd_y = 0;
    while(memory.memory.memory[mem_map.interrupt_flag] == 0) { // Go until we have an HBlank interrupt.
        irq_vblank, irq_stat = PPU.cycle(&ppu, &memory.memory);
    }
    ...
    std.testing.expectEqual(0b0000_0000, memory.memory.memory[mem_map.interrupt_flag]) catch |err| {
