...
pub fn runDMATest() !void {
    ...
    MEMORY.request(&memory, &req);
    std.testing.expectEqual(true, memory.is_running) catch |err| {
        std.debug.print("Failed: DMA is triggered by a write request.\n", .{});
        return err;
    };
    std.testing.expectEqual(start_addr, memory.start_addr) catch |err| {
        std.debug.print("Failed: DMA start address is correct.\n", .{});
        return err;
    };
    std.testing.expectEqual(0, memory.offset) catch |err| {
    ...
    // dma is now done.
    std.testing.expectEqual(false, memory.is_running) catch |err| {
