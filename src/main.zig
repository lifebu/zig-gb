...
export fn init() void {
    ...
    if(build_options.tracy_enabled) {
        ...
        state.alloc = state.allocator.allocator();
    } else {
        state.alloc = state.allocator.allocator();
    }

    // TODO: Maybe change this function to loadOrCreate()?
    state.config.load(state.alloc, def.config_path) catch {
        state.config.save(state.alloc, def.config_path) catch unreachable;
    };
    errdefer state.config.deinit(state.alloc);
    state.config.parseArgs(state.alloc) catch unreachable;
...
fn imgui_cb(file_path: []const u8) void {
    if(state.core) |*loaded_core| {
        loaded_core.deinit(state.alloc, state.config);
    }
    ...
    state.core = .{};
    state.core.?.init(state.alloc, state.config);
}
...
export fn deinit() void {
    ...
    state.platform.deinit(state.alloc, &state.config);
